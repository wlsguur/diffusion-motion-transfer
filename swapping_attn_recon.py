...
class Preprocess(nn.Module):
    def __init__(self, config):
        ...
        # ===== 06.06 swapping =====
        if self.config.save_swapping_attn:
            register_temp_attention_hook(self.unet)
        # ==========================
    ...
    def ddim_sample(self, x, cond):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            for i, t in enumerate(tqdm(timesteps)):
                ...
                # sampling: DDIM 50 step
                output_path = self.config.swapping_save_path
                if not os.path.exists(output_path):
                    os.makedirs(output_path)
                output_path = os.path.join(output_path, f"attentions_step{t}.pkl")
                step = (self.config.n_timesteps + 1) // 50
                if t % step == 0:
    ...
    def extract_latents(self, num_steps, data_path, save_path, inversion_prompt="", negative_prompt=""):
        ...
        # ===== 06.06 swapping ======
        if self.config.save_swapping_attn:
            self.ddim_sample(inverted_x, cond)
...
def register_temp_attention_hook(model):
    for name, module in model.named_modules():
        if (not name or name.endswith(("to_q",  "to_k"))) and "temp_attentions" in name:
