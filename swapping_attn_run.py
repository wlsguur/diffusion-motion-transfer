    ...
    def denoise_step(self, x, t):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            ...
            if self.config.load_swapping_attn and t_sampling == self.config.swapping_step:
                attentions_path = os.path.join(self.config.swapping_load_path, f"attentions_step{t+19}.pkl")
                attentions = {}
            ...
            noise_pred = model(latent_model_input, t, encoder_hidden_states=text_embed_input, return_dict=False)[0]
            # ==================================
...
def register_temp_attention_hook(model, attentions):
    for name, module in model.named_modules():
        if (not name or name.endswith(("to_q",  "to_k"))) and "temp_attentions" in name:
            if name in attentions.keys():
                attn = attentions[name]
                module.register_forward_hook(swapping_attentions(name, attn))
# =================================
